...
class RasterFileToUDB():
    ...
    def RasterToUDB(self):
		...
		DatasourceUDB = smEngine.uds(uPath, datasourceAlias)
                if not DatasourceUDB.has_key(datasourceAlias):
                ...
                for rasterFile in files:
                    ...
                    if DatasourceUDB.ImportImage(datasetName,self.encodeType,self.fileType,rasterFile):
                ...
                if self.isBuildPyramid:
                    datasetCount = DatasourceUDB.GetDatasetCount(datasourceAlias)
                    for datasetIndex in range(datasetCount):
                        datasetName = DatasourceUDB.GetDatasetName(datasourceAlias,datasetIndex)
